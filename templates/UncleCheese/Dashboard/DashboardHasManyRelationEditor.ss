<div class="field dashboard-has-many-editor" data-refresh-url="$Link">
	...
	<% if $Items %>
		<ul class="dashboard-has-many-list dashboard-sortable" data-sort-url="$Link('sort')">
			<% loop $Items %>
				<li id="item-$ID" class="dashboard-has-many-item $EvenOdd clearfix">
					<a class="edit-link" href="$EditLink">$Label</a>
					<a class="delete-link" href="$DeleteLink"><img src="dashboard/images/delete.png" width="16" height="16"/></a>
				</li>
			<% end_loop %>
		</ul>
	<% else %>
